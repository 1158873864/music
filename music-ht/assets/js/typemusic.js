...
var firstID=0;
var theGroup=0;
$.get("assets/php/getmusictype.php" ,async =false,function (data){
    var temp=data.split("\n");
...
function deletequestion(n){
    ...
    if(r) {
        var q = list[firstID + n];
        $.post("assets/php/deletemusictype.php", {number: q[0]}, function (data) {
            alert("删除成功");
            window.location.href = "add_type_music.html";
        })
    }
}
function changetype(n) {
    var q=list[firstID+n];
    $.post("assets/php/deletemusictype.php", {number:q[0]} ,function (data){
        window.location.href="changemusictype.html";
    })
}
